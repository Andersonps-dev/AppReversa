...
def extrair_dados_estoques_wms(link_wms, user_wms, senha_wms, id_depositante=2361178, armazem="insider%", save_path=r"AppReversa\instance"):
    try:
        ...
        if consulta_response.status_code != 200:
            print(f"Falha na consulta de estoque! Status: {consulta_response.status_code}")
            return
        
        print("Consulta de estoque realizada com sucesso!")
        ...
        if download_response.status_code != 200:
            print(f"Falha ao baixar o arquivo CSV! Status: {download_response.status_code}")
            return
        ...
        conn.commit()
        conn.close()
        print(f"Dados salvos com sucesso no banco de dados: {db_path}, tabela: estoque")
...
if __name__ == "__main__":
    from config import LINK_WMS, LOGINS_WMS, SENHAS_WMS
    extrair_dados_estoques_wms(
        link_wms=LINK_WMS,
