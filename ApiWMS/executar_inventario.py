...
class InventoryExecutor:
    ...
    def __init__(self, user1=None, pass1=None, user2=None, pass2=None, locais_banco=None, items_by_location=None):
        ...
        self.user2 = user2
        self.pass2 = pass2
        self.locais_banco = locais_banco or []
    ...
    def create_inventory_scope(self):
        ...
        if login_response.status_code != 200:
            flash(f"Erro: Falha no login com status {login_response.status_code}", "error")
        ...
        if not bearer_token:
            flash("Erro: Token de autenticação não encontrado", "error")
        ...
        if save_response.status_code != 200:
            flash(f"Erro: Falha ao salvar inventário com status {save_response.status_code}", "error")
            raise Exception(f"Failed to save inventory with status {save_response.status_code}")

        inventory_id = save_response.json().get("id")
        if not inventory_id:
            flash("Erro: ID do inventário não retornado", "error")
        ...
        if depositantes_response.status_code != 204:
            flash(f"Erro: Falha ao adicionar depositantes com status {depositantes_response.status_code}", "error")
        ...
        if usuarios_response.status_code != 200:
            flash(f"Erro: Falha ao buscar usuários com status {usuarios_response.status_code}", "error")
        ...
        if usuarios:
            ...
            if add_usuarios_response.status_code != 204:
                flash(f"Erro: Falha ao adicionar usuários com status {add_usuarios_response.status_code}", "error")
        ...
        for local in self.locais_banco:
            ...
            if locais_response.status_code != 200:
                flash(f"Aviso: Falha ao buscar local {local} com status {locais_response.status_code}", "warning")
                continue
        ...
        if not locais:
            flash("Erro: Nenhum local válido encontrado para adicionar", "error")
        ...
        if add_locais_response.status_code != 204:
            flash(f"Erro: Falha ao adicionar locais com status {add_locais_response.status_code}", "error")
        ...
        if liberar_response.status_code != 204:
            flash(f"Erro: Falha ao liberar inventário com status {liberar_response.status_code}", "error")
            raise Exception(f"Failed to release inventory with status {liberar_response.status_code}")

        flash("Inventário criado com sucesso", "success")
        return inventory_id
    ...
    def execute_inventory(self):
        for cred in self.wms_credentials:
            ...
            if login_response.status_code != 302:
                flash(f"Erro: Falha no login do usuário {cred['username']} com status {login_response.status_code}", "error")
            ...
            if not select:
                flash("Erro: Elemento de seleção de inventário não encontrado", "error")
                raise Exception("Inventory select element not found")
            ...
            try:
                ...
                flash(f"Erro: ID do inventário {self.inventory_id} não encontrado na lista", "error")
                raise Exception(f"Inventory ID {self.inventory_id} not found in scope list")
            ...
            for local, items in location_groups.items():
                ...
                if response_op3.status_code != 200:
                    flash(f"Aviso: Falha ao processar local {local} com status {response_op3.status_code}", "warning")
                    continue
                time.sleep(0.5)
                ...
                for item in items:
                    ...
                    if not code or not qty:
                        flash(f"Aviso: Item inválido no local {local}: Código ou quantidade vazios", "warning")
                        continue
                    wms_session.post(
                        inventario_url,
            ...
            flash(f"Inventário executado com sucesso para usuário {cred['username']}", "success")
    ...
    def atualizar_critica(self):
        ...
        response_inventario = api_session.post(
            f'{self.BASE_API_URL}/InventarioService/getInventarios',
            headers=auth_headers,
            json=json_data_getInventario,
        )
        if response_inventario.status_code != 200:
            flash(f"Erro: Falha ao buscar inventários com status {response_inventario.status_code}", "error")
        ...
        response_monitorar = api_session.post(
            f'{self.BASE_API_URL}/InventarioService/getMonitorarInventarios',
            headers=auth_headers,
            json=json_data_monitorar,
        )
        if response_monitorar.status_code != 200:
            flash(f"Erro: Falha ao monitorar inventários com status {response_monitorar.status_code}", "error")
        ...
        response_critica = api_session.post(
            endpoints['atualizar_critica'],
            headers=auth_headers,
            json=json_data_atualizar_critica,
        )
        if response_critica.status_code != 204:
            flash(f"Erro: Falha ao atualizar críticas com status {response_critica.status_code}", "error")
        else:
            flash("Críticas atualizadas com sucesso", "success")
        ...
        response_bloqueio = api_session.post(
            f'{self.BASE_API_URL}/InventarioService/bloquearInventario',
            headers=auth_headers,
            json=json_data_bloqueio_contagem,
        )
        if response_bloqueio.status_code != 204:
            flash(f"Erro: Falha ao bloquear contagem com status {response_bloqueio.status_code}", "error")
        ...
        response_att_estoque = api_session.post(
            f'{self.BASE_API_URL}/InventarioService/atualizarEstoque',
            headers=auth_headers,
            json=json_data_atualizar_estoque,
        )
        if response_att_estoque.status_code != 204:
            flash(f"Erro: Falha ao atualizar estoque com status {response_att_estoque.status_code}", "error")
        else:
